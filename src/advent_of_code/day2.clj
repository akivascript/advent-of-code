...
(def input "1136	1129	184	452	788	1215	355	1109	224	1358	1278	176	1302	186	128	1148
...
(defn checksum
  [input]
  (loop [result 0
         sheet (mapv vector (clojure.string/split-lines input))]
    (if (= (first sheet) nil)
      result
      (let [row (apply #(clojure.string/split % #"\t") (first sheet))
            row' (mapv #(Integer/parseInt %) row)]
        (recur (+ result (- (apply max row') (apply min row')))
               (rest sheet))))))
