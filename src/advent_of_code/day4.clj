...
(def input "pphsv ojtou brvhsj cer ntfhlra udeh ccgtyzc zoyzmh jum lugbnk
...
(defn count-valid
  [input]
  (loop [valid-count 0
         passphrases (mapv vector (clojure.string/split-lines input))]
    (if (= (first passphrases) nil)
      valid-count
      (let [passphrase (apply #(clojure.string/split % #" ") (first passphrases))
            valid? (= (count passphrase)
...
(defn count-valid-anagram
  [input]
  (loop [valid 0
         passphrases (clojure.string/split-lines input)]
    (if (nil? (first passphrases))
      valid
      (let [passphrase (clojure.string/split (first passphrases) #" ")]
        (recur (if (apply distinct? (map sort passphrase))
                 (inc valid)
                 valid)
               (rest passphrases))))))
